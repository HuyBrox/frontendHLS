...
// Thêm async thunk cho việc fetch bookmarks
export const fetchBookmarks = createAsyncThunk(
  ...
  async ({ postId }, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        return rejectWithValue(errorData.message || 'Failed to toggle bookmark.');
      }

      const data = await response.json();
      return data.bookmarks || []; // Trả về danh sách bookmarks mới sau khi cập nhật
    } catch (error) {
      return rejectWithValue(error.message);
    }
  }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchProfileData.fulfilled, (state, action) => {
        ...
        state.bookmarks = action.payload.user.bookmarks || [];
      })
      ...
      .addCase(fetchBookmarks.fulfilled, (state, action) => {
        state.loading = false;
        state.bookmarks = action.payload; // Cập nhật danh sách bookmarks từ phản hồi API
      })
