...
import { fetchCurrentUserStories, fetchMyStories } from './../../../store/features/story/storySlice';
...
const Profile = () => {
  ...
  const { userStories, myStories } = useSelector((state) => state.story);

  // Thêm useEffect để kiểm tra trạng thái stories
  useEffect(() => {
    if (profileData?._id) {
      const viewedStatus = sessionStorage.getItem(`storiesViewed_${profileData._id}`);
      setAllStoriesViewed(viewedStatus === 'true');
    }
  }, [profileData?._id]);

  // Thêm useEffect để lưu trạng thái stories viewed
  useEffect(() => {
    if (profileData?._id) {
  ...
  //  useEffect fetch profile của component cha
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (!hasFetchedProfile.current && (userId || youUserId !== myUserId)) {
          ...
          // console.log(data.user.story_archive)
          if (data.success) {
  ...
  // Thêm hàm xử lý khi click vào nút xem story trong modal
  const handleViewStoryClick = async () => {
    if (isOwnProfile) {
      // Nếu là profile của chính mình, sử dụng API my-stories
      await dispatch(fetchMyStories());
      setShowModal(false);
      setShowStoryViewer(true);
    } else {
      // Nếu là profile người khác, giữ nguyên logic cũ
      await dispatch(fetchCurrentUserStories(profileData?._id));
      setShowModal(false);
      setShowStoryViewer(true);
    }
  };

  // Thêm hàm xử lý khi xem xong story
  ...
  return (
    <div className="profile">
      ...
      />}
      {selectedTab === 'saved' && <Bookmark />}
      {selectedTab === 'tagged' && <Tag />}
        ...
        <div className="profile__modal">
          <div className="profile__modal-content">
            ...
            {profileData?.stories?.length > 0 && (
              <button onClick={handleViewStoryClick}>Xem story</button>
            )}
