...
import { fetchGetPost } from "../../../../../store/features/NewPost/newPostSlice"
...
const PreviewProfilePost = ({
  ...
  const { selectedPost } = useSelector((state) => state.newPost);
  ...
  useEffect(() => {
    if (isOpen && post?._id) {
      dispatch(fetchGetPost({ postId: post._id }));
    }
  }, [dispatch, isOpen, post?._id]);
  ...
  return (
    <div className={`fixed inset-0 flex items-center justify-center z-50 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'} transition-all duration-300`} style={{ zIndex: 777 }}>
      ...
      <div className="relative w-full max-w-6xl h-[90vh] rounded-xl flex background_PreviewPost">
        ...
        <div className="flex flex-1 max-h-full">
          ...
          <div className="w-[45%] flex flex-col h-full overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-[#3a3344]">
              <div
              ...
              >
                ...
                />
                <span className="text-white text-sm font-medium">{selectedPost?.author?.username}</span>
              </div>
              <button
