...
export const RightSidebar = ({
  ...
  const { conversationList, unreadCounts, onlineUsers, userStatuses, myUserId, searchResults } = useSelector(state => state.messenger);
  const [minimizedChats, setMinimizedChats] = useState(() =>
    getFromSessionStorage(STORAGE_KEYS.MINIMIZED_CHATS) || []
  );
  const { followingUsers, loading: followLoading } = useSelector(state => state.follow);
  ...
  useEffect(() => {
    dispatch(searchUserMessages(searchValue));
      ...
      <div className="fixed right-0 h-screen flex flex-col Sidebar-right" style={{ width: "340px", borderRadius: "10px", padding: "0 10px" }}>
        ...
        <div className={`${expandSuggestions ? 'flex-1' : ''} overflow-hidden transition-all duration-300`}>
          ...
          <div className={`${expandSuggestions ? 'h-[calc(100vh-100px)]' : ''} overflow-y-auto`}>
            {(expandSuggestions ? suggestedUsers : suggestedUsers.slice(0, 2)).map((user) => (
              <div
                key={user._id}
                className="flex items-center p-3 hover:bg-[#3a3344] cursor-pointer"
              ...
              >
                <div className="flex items-center w-full">
                  <img
                    ...
                    className="w-8 h-8 bg-gray-700 rounded-full mr-3 object-cover"
                  />
                  <div className="font-medium">{user.fullname || user.username}</div>
                  <button
                    className='btn-follow ml-auto text-sm text-[#00bfff] w-[90px] h-[35px]'
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  >
                    {followingUsers.includes(user._id) ? 'Bỏ theo dõi' : 'Theo dõi'}
                  </button>
                </div>
              </div>
            ))}
          ...
          >
            ...
            >
              {searchValue ? (
                  ...
                  >
                    <div className="flex gap-3 items-center relative">
                      <div className="relative">
                        ...
                        />
                        {/* Giữ lại logic online/offline nếu có */}
                ...
                // Hiển thị danh sách cuộc trò chuyện ban đầu
