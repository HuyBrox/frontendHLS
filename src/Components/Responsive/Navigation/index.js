...
const Navigation = () => {
  ...
  // Cập nhật chỉ số activeIndex khi thay đổi đường dẫn
  useEffect(() => {
    const index =
      pathToIndex[location.pathname] || (location.pathname.startsWith("/profile") ? 4 : 0);
    setActiveIndex(index);
  }, [location.pathname]);

  // Cập nhật vị trí thanh chỉ báo
  useEffect(() => {
  ...
  const navigationItems = [
    ...
    {
      path: `/profile/${id}`, // Profile navigation
      icon: (
        <img
          src={profilePicture}
          alt="Profile"
        ...
        {navigationItems.map((item, index) => (
          <li
            ...
            onClick={() => {
              ...
              setActiveIndex(index); // Đảm bảo luôn cập nhật trạng thái
            }}
          >
            <Link to={item.path || "#"}>
              <span>{item.icon}</span>
